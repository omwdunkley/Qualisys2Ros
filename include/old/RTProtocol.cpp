                          ...
                          int nMajorVersion, int nMinorVersion, bool bBigEndian)
{
    ...
    if (mpoNetwork->Connect(pServerAddr, nPort))
    {
        ...
        if (nUDPServerPort != 0)
        {
            ...
            {
				//For Debugging I used printfs instead of the error strings
				//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------
                ...
                if (strncmp("QTM RT Interface connected", mpoRTPacket->GetCommandString(), sizeof("QTM RT Interface connected")) == 0)
                {
                    ...
                    if (SetVersion(mnMajorVersion, mnMinorVersion))
                    {
                        ...
                        if ((mnMajorVersion == 1) && (mnMinorVersion == 0))
                        {
                            if (mbBigEndian)
                            {
                                printf(tTemp, "ByteOrder BigEndian");
                            }
                            else
                            {
                                printf(tTemp, "ByteOrder LittleEndian");
                            }
        ...
        printf("Missing QTM server welcome message.");
        Disconnect();
    }
    else
    {
        ...
        {
            printf( "Check if QTM is running on target machine.");
        }
        else
        {
            printf("%s", mpoNetwork->GetErrorString());
        }
    }
    //printf("failed at the end...\n");
    return false;
} // Connect
...
bool CRTProtocol::SetVersion(int nMajorVersion, int nMinorVersion)
{
    ...
    if (SendCommand(tTemp, pResponseStr, sizeof(pResponseStr)))
    {
        ...
        {
			//printf("Set Version worked");
            return true;
        }

        if (pResponseStr)
        {
            printf( "Respoens %s.", pResponseStr);
        }
        else
        {
    ...
    else
    {
        ...
        printf( "Send Version failed. %s.", tTemp);
    }
    return false;
}
...
//I couldnt find a Linux class like iphlpapi.h so i just commented it
...
{
    //printf("Here in ReceiveRTPacket \n");
    ...
    do 
    {
	//printf("Loop ReceiveRTPacket start \n");
	...
	//printf("Loop ReceiveRTPacket end \n");
        ...
        if (ReceiveRTPacket(eType, true))
        {
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("General") && oXML.IntoElem())
                {
                    ...
                    {
                        msGeneralSettings.nCaptureFrequency = atoi(oXML.GetChildData());
                        if (oXML.FindChildElem("Capture_Time"))
                        {
                            msGeneralSettings.fCaptureTime = (float)atof(oXML.GetChildData());
                            if (oXML.FindChildElem("Start_On_External_Trigger"))
                            {
                                msGeneralSettings.bStartOnExternalTrigger = (oXML.GetChildData().CompareNoCase("true") == 0);
                                bReturn = true;
                           }
                        }
                    }
                    ...
                    if (bReturn && oXML.FindChildElem("External_Time_Base") && oXML.IntoElem())
                    {
                        ...
                        {
                            msGeneralSettings.sExternalTimebase.bEnabled = (oXML.GetChildData().CompareNoCase("true") == 0);
                        }
                        else
                        {
                            bReturn = false;
                    ...
                    if (bReturn && oXML.FindChildElem("Processing_Actions") && oXML.IntoElem())
                    {
                        ...
                        {
                            if (oXML.GetChildData().CompareNoCase("true") == 0)
                            {
                    ...
                    while (bReturn && oXML.FindChildElem("Camera") && oXML.IntoElem())
                    {
                        ...
                        {
                            sCameraSettings.nID = atoi(oXML.GetChildData());
                        }
                        else
                        {
                            bReturn = false;
                        ...
                        {
                            sCameraSettings.bUnderwater = (oXML.GetChildData().CompareNoCase("true") == 0);
                        }

                        if (oXML.FindChildElem("Serial"))
                        {
                            sCameraSettings.nSerial = atoi(oXML.GetChildData());
                        }
                        else
                        {
                            bReturn = false;
                        ...
                        if (oXML.FindChildElem("Video_Exposure") && oXML.IntoElem())
                        {
                            ...
                            {
                                sCameraSettings.nVideoExposure = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nVideoExposureMin = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nVideoExposureMax = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Video_Flash_Time") && oXML.IntoElem())
                        {
                            if (oXML.FindChildElem("Current"))
                            {
                                sCameraSettings.nVideoFlashTime = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Min"))
                            {
                                sCameraSettings.nVideoFlashTimeMin = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Max"))
                            {
                                sCameraSettings.nVideoFlashTimeMax = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Marker_Exposure") && oXML.IntoElem())
                        {
                            if (oXML.FindChildElem("Current"))
                            {
                                sCameraSettings.nMarkerExposure = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Min"))
                            {
                                sCameraSettings.nMarkerExposureMin = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Max"))
                            {
                                sCameraSettings.nMarkerExposureMax = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Marker_Threshold") && oXML.IntoElem())
                        {
                            if (oXML.FindChildElem("Current"))
                            {
                                sCameraSettings.nMarkerThreshold = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Min"))
                            {
                                sCameraSettings.nMarkerThresholdMin = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Max"))
                            {
                                sCameraSettings.nMarkerThresholdMax = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Position") && oXML.IntoElem())
                        {
                            ...
                            {
                                sCameraSettings.fPositionX = (float)atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionY = (float)atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionZ = (float)atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[0][0] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[1][0] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[2][0] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[0][1] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[1][1] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[2][1] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[0][2] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[1][2] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.fPositionRotMatrix[2][2] = (float)atof(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        {
                            sCameraSettings.nOrientation = atoi(oXML.GetChildData());
                        }
                        else
                        {
                            bReturn = false;
                        ...
                        if (oXML.FindChildElem("Marker_Res") && oXML.IntoElem())
                        {
                            ...
                            {
                                sCameraSettings.nMarkerResolutionWidth = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nMarkerResolutionHeight = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Video_Res") && oXML.IntoElem())
                        {
                            if (oXML.FindChildElem("Width"))
                            {
                                sCameraSettings.nVideoResolutionWidth = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Height"))
                            {
                                sCameraSettings.nVideoResolutionHeight = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Marker_FOV") && oXML.IntoElem())
                        {
                            ...
                            {
                                sCameraSettings.nMarkerFOVLeft = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nMarkerFOVTop = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nMarkerFOVRight = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            {
                                sCameraSettings.nMarkerFOVBottom = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Video_FOV") && oXML.IntoElem())
                        {
                            if (oXML.FindChildElem("Left"))
                            {
                                sCameraSettings.nVideoFOVLeft = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Top"))
                            {
                                sCameraSettings.nVideoFOVTop = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Right"))
                            {
                                sCameraSettings.nVideoFOVRight = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                            ...
                            if (oXML.FindChildElem("Bottom"))
                            {
                                sCameraSettings.nVideoFOVBottom = atoi(oXML.GetChildData());
                            }
                            else
                            {
                                bReturn = false;
                        ...
                        if (oXML.FindChildElem("Sync_Out") && oXML.IntoElem())
                        {
                                ...
                                sCameraSettings.eSyncOutMode == ModeActualFreq)
                            {
                                ...
                                {
                                    sCameraSettings.nSyncOutValue = atoi(oXML.GetChildData());
                                }
                                else
                                {
                                    bReturn = false;
                                ...
                                {
                                    sCameraSettings.fSyncOutDutyCycle = (float)atof(oXML.GetChildData());
                                }
                                else
                                {
                                    bReturn = false;
                                ...
                                sCameraSettings.eSyncOutMode != ModeFixed100Hz)
                            {
                                if (oXML.FindChildElem("Signal_Polarity"))
                                {
                                    sCameraSettings.bSyncOutNegativePolarity =
                                        (oXML.GetChildData().CompareNoCase("negative") == 0);
                                }
                                else
                                {
                                    bReturn = false;
        ...
        if (ReceiveRTPacket(eType, true))
        {
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("The_3D") && oXML.IntoElem())
                {
                    ...
                    {
                        unsigned int nNumberOfLabels = atoi(oXML.GetChildData());
                        ...
                        for (unsigned int iLabel = 0; iLabel < nNumberOfLabels; iLabel++)
                        {
                            if (oXML.FindChildElem("Label"))
                            {
                                ...
                                if (oXML.FindChildElem("Name"))
                                {
                                    ...
                                    {
                                        sLabel.nRGBColor = atoi(oXML.GetChildData());
                                    }
...
bool CRTProtocol::Read6DOFSettings()
{
	...
	printf("Get 6D settings...\n");
    if (SendCommand("GetParameters 6D"))
    {
		printf("send command worked...\n");
        if (ReceiveRTPacket(eType, true))
        {
            ...
            if (eType == CRTPacket::PacketXML)
            {
                ...
                // Read 6DOF bodies
                //
                printf("start to read 6d settings...\n");
                if (oXML.FindChildElem("The_6D") && oXML.IntoElem())
                {
                    ...
                    {
                        int nBodies = atoi(oXML.GetChildData());
                        ...
                        for (int iBody = 0; iBody < nBodies; iBody++)
                        {
                            if (oXML.FindChildElem("Body"))
                            {
                                ...
                                if (oXML.FindChildElem("Name"))
                                {
                                    ...
                                    {
                                        s6DBodySettings.vsPoints.clear();
                                        s6DBodySettings.nRGBColor = atoi(oXML.GetChildData());
                                        while (oXML.FindChildElem("Point"))
                                        {
                                            oXML.IntoElem();
                                            if (oXML.FindChildElem("X"))
                                            {
                                                sPoint.fX = (float)atof(oXML.GetChildData());
                                            }
                                            else
                                            {
                                                bReturn = false;
                                            ...
                                            if (oXML.FindChildElem("Y"))
                                            {
                                                sPoint.fY = (float)atof(oXML.GetChildData());
                                            }
                                            else
                                            {
                                                bReturn = false;
                                            ...
                                            if (oXML.FindChildElem("Z"))
                                            {
                                                sPoint.fZ = (float)atof(oXML.GetChildData());
                                            }
                                            else
                                            {
                                                bReturn = false;
            ...
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("Analog"))
                {
                    ...
                    if (mnMajorVersion == 1 && mnMinorVersion == 0)
                    {
                        ...
                        {
                            sAnalogDevice.nChannels  = atoi(oXML.GetChildData());
                            if (oXML.FindChildElem("Frequency"))
                            {
                                sAnalogDevice.nFrequency = atoi(oXML.GetChildData());
                                if (oXML.FindChildElem("Unit"))
                                {
                                    ...
                                    if (oXML.FindChildElem("Range"))
                                    {
                                        ...
                                        if (oXML.FindChildElem("Min"))
                                        {
                                            sAnalogDevice.nMinRange = atoi(oXML.GetChildData());
                                            if (oXML.FindChildElem("Max"))
                                            {
                                                sAnalogDevice.nMaxRange = atoi(oXML.GetChildData());
                    ...
                    else
                    {
                        ...
                        while (oXML.FindChildElem("Device"))
                        {
                            ...
                            {
                                sAnalogDevice.nDeviceID = atoi(oXML.GetChildData());
            ...
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("Force") && oXML.IntoElem())
                {
                    ...
                    while (oXML.FindChildElem("Plate"))
                    {
                        ...
                        {
                            sForcePlate.nID = atoi(oXML.GetChildData());
                        }
                        ...
                        {
                            sForcePlate.nAnalogDeviceID = atoi(oXML.GetChildData());
                        }
                        else
                        {
                            bReturn = false;
                        ...
                        if (oXML.FindChildElem("Frequency"))
                        {
                            sForcePlate.nFrequency = atoi(oXML.GetChildData());
                        }
                        else
                        {
                            bReturn = false;
                        ...
                        {
                            sForcePlate.fLength = (float)atof(oXML.GetChildData());
                        }
                        if (oXML.FindChildElem("Width"))
                        {
                            sForcePlate.fWidth  = (float)atof(oXML.GetChildData());
                        }
                        ...
                        if (oXML.FindChildElem("Location"))
                        {
                            ...
                            if (oXML.FindChildElem("Corner1"))
                            {
                                ...
                                if (oXML.FindChildElem("X"))
                                {
                                    sForcePlate.asCorner[0].fX = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Y"))
                                {
                                    sForcePlate.asCorner[0].fY = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Z"))
                                {
                                    sForcePlate.asCorner[0].fZ = (float)atof(oXML.GetChildData());
                                }
                                oXML.OutOfElem();
                            }
                            if (oXML.FindChildElem("Corner2"))
                            {
                                ...
                                if (oXML.FindChildElem("X"))
                                {
                                    sForcePlate.asCorner[1].fX = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Y"))
                                {
                                    sForcePlate.asCorner[1].fY = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Z"))
                                {
                                    sForcePlate.asCorner[1].fZ = (float)atof(oXML.GetChildData());
                                }
                                oXML.OutOfElem();
                            }
                            if (oXML.FindChildElem("Corner3"))
                            {
                                ...
                                if (oXML.FindChildElem("X"))
                                {
                                    sForcePlate.asCorner[2].fX = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Y"))
                                {
                                    sForcePlate.asCorner[2].fY = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Z"))
                                {
                                    sForcePlate.asCorner[2].fZ = (float)atof(oXML.GetChildData());
                                }
                                oXML.OutOfElem();
                            }
                            if (oXML.FindChildElem("Corner4"))
                            {
                                ...
                                if (oXML.FindChildElem("X"))
                                {
                                    sForcePlate.asCorner[3].fX = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Y"))
                                {
                                    sForcePlate.asCorner[3].fY = (float)atof(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("Z"))
                                {
                                    sForcePlate.asCorner[3].fZ = (float)atof(oXML.GetChildData());
                                }
                                oXML.OutOfElem();
                            }
                            oXML.OutOfElem();
                        ...
                        if (oXML.FindChildElem("Origin"))
                        {
                            ...
                            if (oXML.FindChildElem("X"))
                            {
                                sForcePlate.sOrigin.fX = (float)atof(oXML.GetChildData());
                            }
                            if (oXML.FindChildElem("Y"))
                            {
                                sForcePlate.sOrigin.fY = (float)atof(oXML.GetChildData());
                            }
                            if (oXML.FindChildElem("Z"))
                            {
                                sForcePlate.sOrigin.fZ = (float)atof(oXML.GetChildData());
                            }
                            oXML.OutOfElem();
                        }
                        if (oXML.FindChildElem("Channels"))
                        {
                            ...
                            while (oXML.FindChildElem("Channel"))
                            {
                                ...
                                {
                                    sForceChannel.nChannelNumber = atoi(oXML.GetChildData());
                                }
                                if (oXML.FindChildElem("ConversionFactor"))
                                {
                                    sForceChannel.fConversionFactor = (float)atof(oXML.GetChildData());
                                }
                        ...
                        if (oXML.FindChildElem("Calibration_Matrix"))
                        {
                            ...
                            while (oXML.FindChildElem(strRow))
                            {
                                ...
                                {
                                    sForcePlate.afCalibrationMatrix[nRow][nCol] = (float)atof(oXML.GetChildData());
                                    nCol++;
                                    sprintf(strCol, "Col%d", nCol + 1);
                                }
                                nRow++;
            ...
            if (eType == CRTPacket::PacketXML)
            {
                ...
                if (oXML.FindChildElem("Image") && oXML.IntoElem())
                {
                    while (oXML.FindChildElem("Camera"))
                    {
                        ...
                        if (oXML.FindChildElem("ID"))
                        {
                            sImageCamera.nID = atoi(oXML.GetChildData());
                            ...
                            if (oXML.FindChildElem("Enabled"))
                            {
                                ...
                                if (oXML.FindChildElem("Format"))
                                {
                                    ...
                                    if (oXML.FindChildElem("Width"))
                                    {
                                        sImageCamera.nWidth = atoi(oXML.GetChildData());

                                        if (oXML.FindChildElem("Height"))
                                        {
                                            sImageCamera.nHeight = atoi(oXML.GetChildData());

                                            if (oXML.FindChildElem("Left_Crop"))
                                            {
                                                sImageCamera.fCropLeft = (float)atof(oXML.GetChildData());

                                                if (oXML.FindChildElem("Top_Crop"))
                                                {
                                                    sImageCamera.fCropTop = (float)atof(oXML.GetChildData());

                                                    if (oXML.FindChildElem("Right_Crop"))
                                                    {
                                                        sImageCamera.fCropRight = (float)atof(oXML.GetChildData());

                                                        if (oXML.FindChildElem("Bottom_Crop"))
                                                        {
                                                            sImageCamera.fCropBottom = (float)atof(oXML.GetChildData());
...
void CRTProtocol::AddXMLElementBool(NBC_CMarkup* oXML, _TCHAR* tTag, const bool* pbValue, _TCHAR* tTrue, _TCHAR* tFalse)
{
    if (pbValue)
    {
...
void CRTProtocol::AddXMLElementInt(NBC_CMarkup* oXML, _TCHAR* tTag, const int* pnValue)
{
    if (pnValue)
    {
        CStdString tVal;
...
void CRTProtocol::AddXMLElementUnsignedInt(NBC_CMarkup* oXML, _TCHAR* tTag, const unsigned int* pnValue)
{
    if (pnValue)
    {
        CStdString tVal;
...
void CRTProtocol::AddXMLElementFloat(NBC_CMarkup* oXML, _TCHAR* tTag, const float* pfValue, unsigned int pnDecimals)
{
    if (pfValue)
    {
        CStdString tVal;
